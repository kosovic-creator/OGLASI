...
export default function EditWhiteGoodsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    async function loadAd() {
      try {
        const data = await getWhiteGoodsById(id)
        if (!data) {
          router.push('/bijela-tehnika')
          return
        }
        if (session?.user?.id !== data.userId) {
          router.push(`/bijela-tehnika/${id}`)
          return
        }
        setAd(data)
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      await updateWhiteGoods(session.user.id, id, adData, whiteGoodsData)
      router.push(`/bijela-tehnika/${id}`)
    } catch (err: any) {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      await deleteWhiteGoods(session.user.id, id)
      router.push('/bijela-tehnika')
    } catch (err: any) {
      setError(err.message || 'Došlo je do greške')
      setDeleting(false)
    ...
    <div className="container mx-auto px-4 py-8 max-w-4xl">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex gap-4">
          ...
          <Link href={`/bijela-tehnika/${id}`} className="flex-1">
