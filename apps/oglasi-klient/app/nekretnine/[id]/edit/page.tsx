...
export default function EditRealEstatePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      await updateRealEstate(session.user.id, id, adData, realEstateData)
      router.push(`/nekretnine/${id}`)
    } catch (err: any) {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      await deleteRealEstate(session.user.id, id)
      router.push('/nekretnine')
    } catch (err: any) {
      setError(err.message || 'Došlo je do greške')
      setDeleting(false)
    ...
    <div className="container mx-auto px-4 py-8 max-w-4xl">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex gap-4">
          ...
          <Link href={`/nekretnine/${id}`} className="flex-1">
