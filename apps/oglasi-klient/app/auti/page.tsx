...
import { getVehicles } from '@oglasi/server-actions'
import { PlusCircle, Car, Eye, MapPin, Calendar, Gauge, Fuel } from 'lucide-react'
import Image from 'next/image'

export default async function AutomobiliPage() {
  const vehicles = await getVehicles({ take: 20 })
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-gray-600 mt-2">Pregledajte ponudu automobila, teretnih vozila i motocikala</p>
        </div>
        <Link href="/auti/dodaj">
      ...
      {vehicles.length === 0 ? (
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-16">
            ...
            <h3 className="text-xl font-semibold text-gray-900 mb-2">Nema automobila</h3>
            <p className="text-gray-600 mb-6">Trenutno nema objavljenih automobila.</p>
            <Link href="/auti/dodaj">
              <Button>
                <PlusCircle className="h-4 w-4 mr-2" />
                Dodaj prvi automobil
              </Button>
            </Link>
          </CardContent>
        </Card>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {vehicles.map((ad) => {
            ...
            return (
              <Link key={ad.id} href={`/auti/${ad.id}`}>
